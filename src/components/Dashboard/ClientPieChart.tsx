...
export const ClientPieChart: React.FC<ClientChartTypes> = ({ textContent = '' }) => {
  ...
  React.useEffect(() => {
    ;(async () => {
      if (firmId && isFirm) {
        await dashboardActions.FirmClientsData({
          firmId,
          scope: Period,
        })
      }
  ...
  const data = [
    { name: 'New Clients', value: dashboardState?.dashboard_clients_firm?.new },
    { name: 'Returning Client', value: dashboardState?.dashboard_clients_firm?.returning },
  ]
  ...
  return (
    <div className="bg-white py-4 px-5 module w-full">
      ...
      <WrapperStyled>
        {dashboardState?.dashboard_clients_firm?.new ||
        dashboardState?.dashboard_clients_firm?.returning !== 0 ? (
          <PieChart height={400} width={200}>
            <Pie
              ...
              label={renderCustomizedLabel(dashboardState?.dashboard_clients_firm?.total)}
              legendType="circle"
              outerRadius={80}
            >
