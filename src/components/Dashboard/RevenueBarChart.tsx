...
export const RevenueBarChart: React.FC<RevenueChartTypes> = ({ textContent = '' }) => {
  ...
  React.useEffect(() => {
    ;(async () => {
      if (firmId && isFirm) {
        await dashboardActions.FirmRevenuData({
          firmId,
  ...
  const data = dashboardState?.dashboard_revenue_firm?.map((d) => ({
    name: d.month,
  ...
  return (
    <div className="bg-white py-4 px-5 module w-full h-full">
      ...
      <WrapperStyled>
        <ResponsiveContainer>
          {dashboardState?.dashboard_revenue_firm?.length !== 0 ? (
            <BarChart
              data={data}
              height={400}
